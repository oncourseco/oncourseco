...
func dumpStats(events *calendar.Events) (err error) {
	...
	if len(events.Items) > 0 {
		for _, i := range events.Items {
			...
			if i.Start.DateTime != "" {
				start, _ = time.Parse("2000-01-22 24:30:30", i.Start.DateTime)
				end, _ = time.Parse("2000-01-22 24:30:30", i.End.DateTime)
			} else {
				start, _ = time.Parse("2000-01-22", i.Start.DateTime)
			}
